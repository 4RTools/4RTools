...
{
    public partial class StalkerForm : Form, IObserver
    {
        public static int TOTAL_MACRO_LANES = 3;
        public StalkerForm(Subject subject)
        {
            subject.Attach(this);
            InitializeComponent();
            configureMacroLanes();
        ...
        private void UpdatePanelData(int id)
        {
            try
            {
                ...
                foreach (string cbName in names)
                {
                    ...
                    Control[] cbClicks = group.Controls.Find($"{cbName}click", true); // Clicks
                    if (cbClicks.Length > 0)
                    {
                        CheckBox checkbox = (CheckBox)cbClicks[0];
                        checkbox.Checked = chainConfig.macroEntries[cbName].hasClick;
                    }
                }
            }
            catch { };
